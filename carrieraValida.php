      ...
      <div class="row">
        ...
        <div id="rigaContainer" class="row">
            <div id="colonna1" class="col-6 m-auto">
                <form action="carrieraValida.php" method="POST">
                    ...
                    <div id="risultato" class="row m-auto">
                        <?php
                            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['richiedi'])) {
                               ...
                               $query = "WITH dataMassima AS (
                                            SELECT t.id_carriera, t.id_insegnamento, MAX(t.DATA) as data
                                            FROM universita.trascrizione t INNER JOIN universita.trascrizione t2
                                            ON t.id_carriera = t2.id_carriera AND t.id_insegnamento = t2.id_insegnamento
                                            GROUP BY t.id_carriera, t.id_insegnamento)
                                        ...
                                        WHERE s.matricola = $1 AND t.esito";
                    ...
                    <div id="risultato" class="row m-auto">
                        <?php
                            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['richiedi'])) {
                                ...
                                // Query di inserimento
                               $query = "WITH dataMassima AS (
                                            SELECT c.id_carriera, c.id_insegnamento, MAX(c.data) as data
                                            FROM universita.curriculum c
                                            GROUP BY id_carriera, id_insegnamento)

                                        SELECT c.nome_esame, c.esito, c.voto, c.data
                                        FROM universita.curriculum c
                                        INNER JOIN universita.storico_studente s ON s.id_storico_carriera = c.id_carriera 
                                        INNER JOIN dataMassima d ON c.id_insegnamento = d.id_insegnamento AND c.data = d.data
                                        WHERE s.matricola = $1 AND c.esito";
